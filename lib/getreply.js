  ...
  async.filter(replies, filterHandle, function (filteredReplies) {
    ...
    filterRepliesBySeen(filteredReplies, user, function afterFilterRepliesBySeen(err, reply) {
      if (err) {
        ...
        processTags.replies(reply, user, options, function (err2, processedReplyString, props, replyObj) {

          if (processedReplyString === "") {
            callback();
          } else if (replyObj) {
            // replyObj is set when we have a respond or redirect function
            var threadsArr = [];

            if (_.isEmpty(replyObj.subReplies)) {
              threadsArr = processTags.threads(replyObj.string);
            } else {
              threadsArr[0] = replyObj.string;
            ...
            var threadsArr = processTags.threads(processedReplyString);
            callback({
              id: reply.id,
