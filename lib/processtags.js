...
var replies = function (replyObj, user, options, callback) {
  ...
  replace(reply, WORDNET_REGEX, processHelpers.wordnetReplace, function(err, wordnetReply) {
    ...
    var augmentCallbackHandle = function(err, replyString, messageProps, getReplyObject) {
      if (err) {
        ...
        if (_.isEmpty(getReplyObject)) {
          ...
          // TODO load the replyID and replace it with the existing reply.reply

          newReplyObject = replyObj;
          newReplyObject.reply.reply = getReplyObject.string;
        ...
        debug("Return back to replies to re-process for more tags");
